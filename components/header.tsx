...
export default function Header() {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-16 left-0 h-full md:w-80 w-screen px-4 bg-gray-900 text-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out flex flex-col ${
          !isOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <nav className="flex flex-col items-center gap-6 py-3">
          {navLink.map((items) => (
            <Link className="font-medium" key={items.title} href={items.src}>
              {items.title}
            </Link>
          ))}
        </nav>
